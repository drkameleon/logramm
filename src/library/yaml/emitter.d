...
import library.yaml.anchor;
import library.yaml.encoding;
import library.yaml.escapes;
import library.yaml.event;
import library.yaml.exception;
import library.yaml.fastcharsearch;
import library.yaml.flags;
import library.yaml.linebreak;
import library.yaml.queue;
import library.yaml.style;
import library.yaml.tag;
...
{
    private:
        alias library.yaml.tagdirective.TagDirective TagDirective;
        ...
        void writeDoubleQuoted() @system
        {
            ...
            {   
                     ...
                     ((c >= '\xA0' && c <= '\uD7FF') || (c >= '\uE000' && c <= '\uFFFD'))))
                {
                    ...
                    {
                        auto appender = appender!string();
                        if((c in library.yaml.escapes.toEscapes) !is null)
                        {
                            appender.put('\\');
                            appender.put(library.yaml.escapes.toEscapes[c]);
                        }
                        else
                        {
