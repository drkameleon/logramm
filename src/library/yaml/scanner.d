...
import library.yaml.reader;
import library.yaml.style;
import library.yaml.token;
        ...
        void scanFlowScalarNonSpaces(const ScalarStyle quotes, const Mark startMark) @system
        {
            for(;;)
            {
                ...
                else if(quotes == ScalarStyle.DoubleQuoted && c == '\\')
                {
                    reader_.forward();
                    c = reader_.peek();
                    if((c in library.yaml.escapes.fromEscapes) !is null)
                    {
                        appender_.put(library.yaml.escapes.fromEscapes[c]);
                        reader_.forward();
                    }
                    else if((c in library.yaml.escapes.escapeHexCodes) !is null)
                    {
                        length = library.yaml.escapes.escapeHexCodes[c];
                        reader_.forward();
